...
const float g_rainfactors[370] = float[](
...
void rainResponse(vec3 lightVector, float lightIntensity, vec3 lightColor, vec3 eyeVector, bool fallOffFactor, inout vec4 rainResponseVal)
{
    ...
    if(fallOff > 0.01 && lightIntensity > 0.01 )
    {
		...
		//AddressU = Wrap;
		//AddressV = Wrap;
        float col1 = texture(rainTextureArray, tex1).r * g_rainfactors[texIndicesV1.x];
